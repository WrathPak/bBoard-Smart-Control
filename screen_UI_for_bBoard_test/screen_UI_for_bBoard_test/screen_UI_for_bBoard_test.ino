...
void setup() {
  ...
  //Setup UART port
  Serial1.begin(9600);
  start_tacho = measuredValues.tachometer;
...
void ride_screen()
{
  ...
  current_rpm = round(measuredValues.rpm);
  current_tacho = measuredValues.tachometer;
  ...
  else
  {
   ...
   // in_screen = false;
    ...
    if( y > 270 && y < 400)  // ride, lights, or drive stats
    {
      ...
      if( x > 205 && x < 300)  // board stats button!
      {
        current_screen = 2;
        in_screen=false;
      }
...
void speedcheck()
{
  ...
  current_mph = round(wheel_size * 3.14 * (current_rpm/9) * 60) / 5280;
 
   if (prev_mph != current_mph)
  {
    prev_mph = current_mph;
    tft.fillRect(40, 140, 150, 150, BLACK);

    tft.setCursor(40, 140);
    tft.setTextColor(HX8357_WHITE);
    tft.setTextSize(8);
    tft.println(current_mph);

   // myFile = SD.open("example.txt", FILE_WRITE);
    
   // myFile.println(current_speedval);
...
void distancecheck()
{
    ...
    float tacho_diff = (wheel_size * 3.14 * ((current_tacho-start_tacho)/30)/5280);
    
    tft.fillRect(300, 140, 150, 150, BLACK);
    tft.setTextSize(4);
    tft.setCursor(300, 140);
    tft.println(tacho_diff , 2);
